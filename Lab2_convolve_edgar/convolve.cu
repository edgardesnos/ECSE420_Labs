...
__global__ void convolutionKernel(unsigned char* out, unsigned char* in, float* wm, unsigned int width, unsigned int size)
{
    ...
    /*if (idx < (width - 2) * (size/(4 * width) - 2) * 4) {
        out[idx] = in[idx];
    }

    out[(width - 2) * (size / (4 * width) - 2) * 4] = in[idx];*/

    if (idx >= 4 * width && idx < 4 * width * (size / width - 1) && idx % (4 * width) >= 4 && idx % (4 * width) < 4 * (width - 1)) {

        //out[idx - 4 * width - 4 - 8 * (idx / (4 * width))] = in[idx];

        if (idx % 4 == 3) {
            out[idx - 4 * width - 4 - 8 * (idx / (4 * width))] = in[idx];
        }
        else {
            float out_val = wm[0] * (in[idx] - 4 * width - 4) + wm[1] * (in[idx] - 4 * width) + wm[2] * (in[idx] - 4 * width + 4);
            out_val += wm[3] * (in[idx] - 4) + wm[4] * in[idx] + wm[5] * (in[idx] + 4);
            out_val += wm[6] * (in[idx] + 4 * width - 4) + wm[7] * (in[idx] + 4 * width) + wm[8] * (in[idx] + 4 * width + 4);
            out[idx - 4 * width - 4 - 8 * (idx / (4 * width))] = max(0, min(255, (int) out_val));
        }
    }
    // Else do nothing
...
int main(int argc, char** argv) {
    ...
    printf("Convolution computation time: %f\n", timeElapsed);
